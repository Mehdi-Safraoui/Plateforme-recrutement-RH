...
exports.getEmploiById = (req, res) => {
  const emploiId = req.params.id; // Récupérer l'ID de l'emploi à partir des paramètres de la requête
  Emploi.findByPk(emploiId)
    .then(emploi => {
      if (!emploi) {
        return res.status(404).send({ message: "L'emploi n'a pas été trouvé." });
      }
